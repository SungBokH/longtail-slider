...
export default function BarChartMagnitude({
  ...
  // We’ll also store some snippet-connector line coordinates
  let connectingLines = [];
  ...
  // The group that holds our snippet is placed below the chart+brush
  const snippetBase = marginTop + height + brushHeight + brushOffset + 30;
  const topAxisG = svg.append("g")
    .attr("class", "top-linear-axis")
    .attr("transform", `translate(${marginLeft}, ${snippetBase})`);

  // Use axisBottom for the snippet
  topAxisG.append("g")
  ...
  // Red boundary markers in snippet
  topAxisG.selectAll(".prism-boundary-marker")
    ...
    .each(function(d) {
      ...
      connectingLines.push({ domainValue: d, xSnip: xPos, ySnip: snippetHeight - barLength });
    });

  // Label above top of the red bar
  topAxisG.selectAll(".snippet-bar-label")
    ...
    .attr("y", snippetHeight - barLength - 12)
    .attr("fill", "red")
  ...
  // For consistency, percentile bars in snippet
  ...
  // --------------------------------------------------------------------------
  // ADJUSTMENT: The start of the red dashed line is always the domain boundary,
  // i.e. boundaries[i-1], rather than a domainVal from invertChartXPos(...).
  // So the snippet top is *constant*, never drifting with the prism.
  // --------------------------------------------------------------------------
  function updatePrisms() {
    ...
    // Remove old snippet lines
    svg.selectAll(".snippet-to-prism-line").remove();

    // For each active prism boundary, draw a line from the snippet boundary's top (constant)
    // to the prism's bottom-right corner on the x-axis
    for (let i = 1; i < xBins.length - 1; i++) {
      if (!boundaryHover[i]) continue;

      // Prism's bottom-right corner in absolute coords
      let xBinFinal = xBins[i];
      if (newPrism && boundaryHover[i]) {
        xBinFinal -= rectWidth;
      }
      const prismCornerX = marginLeft + xBinFinal + rectWidth;
      const prismCornerY = marginTop + height;

      // Instead of invertChartXPos, we fix the snippet top domain as boundaries[i-1]
      // so the snippet top remains constant for that boundary.
      // i=1 => boundaries[0], i=2 => boundaries[1], etc.
      const domainVal = boundaries[i - 1]; // anchor to the domain boundary
      const snippetBarTopX = marginLeft + globalXScale(domainVal);
      const snippetBarTopY = snippetBase + (snippetHeight - barLength);

      // Now draw the line
      svg.append("line")
        .attr("class", "snippet-to-prism-line")
        .attr("x1", snippetBarTopX)
        .attr("y1", snippetBarTopY)
        .attr("x2", prismCornerX)
        .attr("y2", prismCornerY)
        .attr("stroke", "red")
