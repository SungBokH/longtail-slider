...
export default function BarChartMagnitude({
  ...
  function drawCustomGridLines() {
    ...
    customGridY.forEach(binCount => {
      ...
      // If 0 <= yPos <= height, we draw a sub-line.
      for (let i = 0; i < yScales.length; i++) {
        ...
        if (yPos >= yForPrismTop) {
          ...
          const xLeftCorner = xBins[i];
          const xRightCorner = xBins[i] + rectWidth;
          const xLeft = apexX + t * (xLeftCorner - apexX);
          const xRight = apexX + t * (xRightCorner - apexX);
  ...
  function createPrism(x, y, dragBehavior) {
    ...
    group.append("polygon")
      .attr("class", "prism")
      .attr("points",
        // apex is (x + rectWidth/2, y - apexOffsetTop)
  ...
  // ---------------------------------------------------
  // Draw a "horizontal" line that spans each sub-chart
  // at a given bin count. Steps through the prism apex.
  // ---------------------------------------------------
  node.drawHorizontalLine = function (binCountValue) {
    // Remove any old horizontal line we might have drawn before
    g.selectAll(".horizontal-value-line").remove();

    // If null/undefined is passed, just remove and return
    if (binCountValue == null) {
      return node;
    }

    // We'll put all the segments for this line in one group
    const hLineGroup = g.append("g")
      .attr("class", "horizontal-value-line");

    // For each sub-chart segment, figure out where the line goes:
    for (let i = 0; i < yScales.length; i++) {
      const yScale = yScales[i];
      const yPos = yScale(binCountValue);

      // If out of bounds, skip
      if (yPos < 0 || yPos > height) continue;

      // The rectangular part of the segment extends:
      //   from xBins[i] + (i>0 ? rectWidth : 0) to xBins[i+1].
      const segmentLeft = xBins[i] + (i > 0 ? rectWidth : 0);
      const segmentRight = xBins[i + 1];
      ...
      if (yPos >= yForPrismTop) {
        ...
        hLineGroup.append("line")
          .attr("class", "custom-grid-line")  // re-use dashed style
          .attr("x1", segmentLeft)
          .attr("x2", segmentRight)
          .attr("y1", yPos)
          .attr("y2", yPos);
